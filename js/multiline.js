...
//Formatter functions for the axes
    ...
    chartObj.formatAsDate = d3.format("%y-%b-%d");
...
//Create axis
    chartObj.xAxis = d3.svg.axis().scale(chartObj.xScale).orient("bottom").ticks(13); //< Can be overridden in definition

    chartObj.yAxis = d3.svg.axis().scale(chartObj.yScale).orient("left"); //< Can be overridden in definition
...
// Change chart size according to window size
    ...
    chartObj.render = function () {
        ...
        chartObj.svg.append("g").attr("class", "y axis").call(chartObj.yAxis).append("text").attr("class", "label").attr("transform", "rotate(-90)").attr("y", -80).attr("x", -chartObj.height / 2).attr("dy", ".71em").style("text-anchor", "middle").text(chartObj.yAxisLable);
        ...
        // Year label
        focus.append("text").attr("class", "focus year").attr("x", 9).attr("y", -5);
        // Focus line
